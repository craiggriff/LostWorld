...
void Splash::CreateOne(float x, float y, float z, DirectX::XMFLOAT4 col, int start_delay)
{
	...
	do
	{
		...
		if (true)//(spark[current_point].bActive == 0)
		{
			...
			spark[current_point].mom.setX(0.5f - (rand() % 100) / 50.0f);
			spark[current_point].mom.setY(0.5f + (rand() % 100) / 50.0f);
			spark[current_point].mom.setZ(0.5f - (rand() % 100) / 50.0f);
...
void Splash::Initialize(Level* pp_Level, bool _bInstanced)
{
	p_Level = pp_Level;

	spark = new Splash_t[1000];
	...
	m_Texture = m_Resources->m_Textures->LoadTexture("spark");
...
Concurrency::task<void> Splash::Update(float timeTotal, float timeDelta)
{
	return Concurrency::create_task([this, timeDelta]
	{
		...
		if (current_index>0)
		{
			qsort(p_index, current_index, sizeof(part_index), SplashSortCB);
		}
	...
	auto this_task = concurrency::create_task([this]
	{
		...
		if (true) //(total_active<(m_maxParticles*20)-1)
		{
			...
			for (i = 0; i<current_index; i++)
			{
				...
				if (spark[p_index[i].part_no].life<2.0f)
				{
					...
					col.w = -1.5f + (spark[p_index[i].part_no].life*0.1f);
