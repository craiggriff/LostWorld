...
void steam::Initialize(Level* pp_Level, bool _bInstanced)
{
	p_Level = pp_Level;

	spark = new steam_t[1000];
...
Concurrency::task<void> steam::Update(float timeTotal, float timeDelta)
{
	return Concurrency::create_task([this, timeDelta]
	{
		...
		if (current_index > 0)
		{
			...
			std::sort(p_index.begin(), p_index.end(), steam_compare{});
		}
...
void steam::SetSplashLigtness(float _splash_lightness)
{
