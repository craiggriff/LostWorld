...
void DeferredBuffersClass::SetRenderTargets(ID3D11DeviceContext* deviceContext)
{
	...
	deviceContext->OMSetRenderTargets(DEFERRED_BUFFER_COUNT, m_renderTargetViewArray, m_depthStencilView);
...
void DeferredBuffersClass::SetRenderTargets(ID3D11DeviceContext* deviceContext, ID3D11DepthStencilView* stencil)
{
	// Bind the render target view array and depth stencil buffer to the output render pipeline.
	deviceContext->OMSetRenderTargets(DEFERRED_BUFFER_COUNT, m_renderTargetViewArray, stencil);
...
Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> DeferredBuffersClass::GetShaderResourceView(int view)
{
