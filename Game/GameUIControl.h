...
namespace Game
{
	...
	public interface class IGameUIControl
	{
		...
		void SetDiceTotal(int dice);
		void SetDiceTextureNum(int dice);
		void SetFeltTextureNum(int dice);

		void SetLevel(int _level);

		void SetDesigner(bool _bDesign);

		void SetInfoVal(int val_id, float val);

		void ShowTotal();
		void HideTotal();

		void CarControlsVisible(bool _bVisible);
		void PlayerControlsVisible(bool _bVisible);

		void SetFPS(int val);
		void SetTimer(double t_val);

		void SetCols(float ar, float ag, float ab, float dr, float dg, float db, float dx, float dy, float dz, float sr, float sg, float sb, float sp);
