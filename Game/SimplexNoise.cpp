...
*/
static inline uint8_t l_hash(int32_t i) {
...
float SimplexNoise::noise(float x) {
	...
	t0 *= t0;
	n0 = t0 * t0 * grad(l_hash(i0), x0);

	// Calculate the contribution from the second corner
	float t1 = 1.0f - x1*x1;
	//  if(t1 < 0.0f) t1 = 0.0f; // not possible
	t1 *= t1;
	n1 = t1 * t1 * grad(l_hash(i1), x1);
...
float SimplexNoise::noise(float x, float y) {
	...
	if (t0 < 0.0f) {
		...
		t0 *= t0;
		n0 = t0 * t0 * grad(l_hash(i + l_hash(j)), x0, y0);
	}
	...
	if (t1 < 0.0f) {
		...
		t1 *= t1;
		n1 = t1 * t1 * grad(l_hash(i + i1 + l_hash(j + j1)), x1, y1);
	}
	...
	if (t2 < 0.0f) {
		...
		t2 *= t2;
		n2 = t2 * t2 * grad(l_hash(i + 1 + l_hash(j + 1)), x2, y2);
	}
