...
if (normal_height>0.0f)//normalLvl == 1.0f)
{
	...
	if (input.cam_dist < 25.0f)
	{
		...
		if (input.tex_blend > 0.0f)
		{
			normalMap = lerp(normalMap, normalArray[1].Sample(SampleType, float3(input.tex, 1.0f)), input.tex_blend);
		}

		if (input.tex_blend2 > 0.0f)
		{
			normalMap = lerp(normalMap, normalArray[2].Sample(SampleType, float3(input.tex, 1.0f)), input.tex_blend2);
		}
	...
	if (distance < lights[i].radius)
	{
		...
		if (lightIntensity > 0.0f)
		{
			...
			if (lights[i].spot > 0.0f)
			{
				...
				{
					depthValue = depthTexture.Sample(SampleType, projectTexCoord).r;
