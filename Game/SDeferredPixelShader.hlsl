...
TextureCubeArray pointshadowMapCubeArray[MAX_POINT_SHADOWS] : register(t6);
...
float4 main(PixelInputType input) : SV_TARGET
{
	...
	[unroll(MAX_POINT_LIGHTS)]for (int i = 0; i < numLights; i++)
	{
				...
				else
				{
					...
					if (lightIntensity > 0.0f)
					{
						...
						if (lights[i].shadow_cube_index > 0)
						{
							...
							{
								if (lights[i].shadow_cube_index == j + 1)
								{
